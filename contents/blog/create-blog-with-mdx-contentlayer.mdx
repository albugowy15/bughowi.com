...
Next, we need to configure our `next.config.js{:js}` file to hook Contentlayer into the `next dev{:shell}` and `next build{:shell}` processes.
...
Creat a schema for your MDX content. To do this, you need to create a new file called `contentlayer.config.js{:js}` in root folder of Next.js project. Here I create a schema for my blog post content.
...
This configuration will generate a single document type named `Post{:js}`. These documents are expected to be `.mdx{:js}` files that lives within a blog directory in my project. And data objects generated from these files will have the following properties:
