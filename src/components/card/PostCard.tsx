	...
	return (
		<div className="bg-darkSecondary rounded-md overflow-hidden border-2 border-slate-300 border-opacity-40 hover:scale-[1.02] transition-transform ease-in-out">
			<div className="w-full h-48 relative overflow-hidden">
				<Image fill src={post.thumbnail} className="object-cover" alt={post.title} />
			</div>
			<div className="p-3 flex flex-col items-stretch justify-between">
				<div>
					<div className="flex gap-2 flex-wrap">
						{post.categories?.map((category, key) => (
							<p key={key} className="bg-blueAccent text-slate-100  p-1 text-sm rounded-md">
								{category.name}
							</p>
						))}
					</div>
					<div className="py-2" />
					<h3 className="font-bold text-2xl inline hover:text-blueAccent transition-colors">
