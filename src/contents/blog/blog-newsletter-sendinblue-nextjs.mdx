...
First, create a new component called `NewsletterForm.tsx{:ts}`. Start creating your own newsletter form component, or you can simply copy this code.

```tsx showLineNumbers title="./components/NewsletterForm.tsx"
const NewsletterForm = () => {
  return (
    <div className="bg-slate-900 w-full max-w-xl rounded border border-gray-700 p-5 mx-auto">
      ...
      <form className="w-full mx-auto mt-1" method="post" target="_blank">
...
![image](https://user-images.githubusercontent.com/49820990/213977560-3891bdbf-46de-4a31-b066-9f339ffa039e.png)
...
Below is clear explanation for the `useNewsletter{:js}` hooks.
The hook starts by importing the `useState{:js}` and `useRef{:js}` hooks from the React library. `useState{:js}` is used to manage the state of the form, and `useRef{:js}` is used to keep track of the input element in the form.

Then, it declares a state variable called `form{:ts}` that is initialized with an object that has two properties, `state: Form.Initial{:ts}` and `message: undefined{:ts}`. The state property is used to keep track of the status of the form, it can be `Form.Initial{:ts}`, `Form.Loading{:ts}`, `Form.Success{:ts}` or `Form.Error{:ts}`. The `message{:js}` property is used to display the message depending of the status of the form.
...
The hook also declares a variable called `inputEl{:js}` which is initialized with the `useRef{:js}` hook. This variable is used to keep track of the email input element in the form.
...
Next, the hook exports a function called `subscribe{:js}` that handles the form submission. This function takes one parameter e of type `React.FormEvent{:js}`, which is the event object that is passed to the function when the form is submitted.

Inside the function, the first thing it does is calling the `preventDefault{:js}` method on the event object to prevent the page from refreshing when the form is submitted. Then it changes the state of the form to `Form.Loading{:js}`
...
After that, it uses the fetch API to make a POST request to the endpoint `/api/newsletter{:js}` with the email entered in the input field as the body of the request. The headers are set to `Content-Type: application/json.{:js}`

Once the fetch request is complete, it parses the response as json and destructures the object to get the value of the error property. If the error property is not undefined, it means that the request failed, it updates the state of the form to `Form.Error{:ts}` and sets the message to the error returned.
...
If there was no error, it sets the value of the input field to empty string, and updates the state of the form to `Form.Success{:ts}` and sets the message to _"Success! You've been added to the list!"_
...
Finally, the hook returns an object containing the `subscribe{:js}` function, the `inputEl{:js}` variable and the form state variable. These are the values that are needed by the component that uses this hook to render the form and handle the form submission.
...
Create `.env{bash}` file in root folder of your project. In the .env file, add a new variable called `SENDINBLUE_API_KEY{:js}` and set it to your Sendinblue API key. For example:
...
To handle the form submission from the `useNewsletter{:js}` hook, you will need to create an API route in your Next.js application that will handle the POST request made by the hook. Here's an example of how you might create such an API route:
      ...
      >
        <div className="flex flex-wrap w-full items-center justify-center gap-2">
          <input
            ...
            ref={inputEl}
            id="email"
            className="flex-auto bg-slate-700 border border-slate-700 text-slate-400 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block p-2"
            placeholder="name@gmail.com"
            required
          />
          <button
            type="submit"
            className="flex-1 w-28 p-1.5 text-center text-slate-200 bg-blue-600 rounded font-bold hover:bg-blue-500 transition-colors"
          >
