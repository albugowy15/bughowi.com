...
Let's create a string `s1{:rs}` and then create another variable `s2{:rs}` and assign `s1{:rs}` value to it.
...
What happens to this string is not a copy. At this point the value for `s1{:rs}` is moved to `s2{:rs}` because only one variable can own the value. If we try to go ahead and use `s1{:rs}` after this point, we get a compiler error: **_borrow of moved value s1_**.
...
In the code above, it is evident that we are moving the value of `s1{:rs}` to `s2{:rs}`. The concept of moving implies the transfer of ownership from one variable to another. In the given example, we transfer ownership from `s1{:rs}` to `s2{:rs}`, which means that `s2{:rs}` now owns the value, and the compiler considers `s1{:rs}` uninitialized. Consequently, attempting to use `s1{:rs}` after the move would result in a compilation error.
...
If we pass `s1{:rs}` to that function, `s1{:rs}` is moved into the local variable `s{:rs}` in `do_something(){:rs}`, which means we can't use `s1{:rs}` anymore because it got moved!

So what do we do?

One option is to move it back when we're done. We'll just make `s1{:rs}` mutable, add a return type to `do_something(){:rs}`, and then return `s{:rs}` as the tail expression, which gets moved back out of the function and used to reinitialize `s1{:rs}`.
...
Here's our `do_something(){:rs}` function again, only this time it takes a reference to a string. The ampersand before that type indicates a reference to a type. When we call `do_something(){:rs}` we pass it a reference to `s1{:rs}`, and `s1{:rs}` retains ownership of the value. `do_something(){:rs}` borrows a reference to the value. The reference (not the value) gets moved into the function. At the end of the function the reference goes out of scope, and the reference gets dropped, so our borrowing ends at that point. After the function call, we can use `s1{:rs}` like normal, because the value never moved.

Under the hood, when we create a reference to `s1{:rs}`, Rust creates a pointer to `s1{:rs}`, but you will almost never talk about pointers in Rust because the language automatically handles their creation and destruction for the most part, and makes sure they're always valid using a concept called **_lifetimes_**. Lifetimes can be summed up as a rule that references must always be valid, which means the compiler won't let you create a reference to outlives the data it is ultimately referencing, and you can never point to null.
...
The syntax for a mutable reference is a little special: ampersand `*{:rs}`, `mut{:rs}`, space, variable or type.

Now wait, why didn't we have to dereference the mutable reference in order to alter `s{:rs}` in the `do_stuff(){:rs}` method?
...
If we manually dereferenced `s{:rs}`, it would look like this:
...
Here I'm dereferencing `s{:rs}` so that I can replace the entire value.

Let's stop and go over what references look like again. If you have the variable `x{:rs}`, then :

- `&x{:rs}` -> Create an immutable reference to that variable value
- `&mut x{:rs}` -> Create a mutable reference to a that variable value

Similarly with types, if `i32{:rs}` is the type of your value, then:

- `&i32{:rs}` -> This is the type of your immutable reference
- `&mut i32{:rs}` -> This is the type of your mutable reference
...
Then dereferencing `x{:rs}` gives you mutable access to the value.

And if `x{:rs}` is an immutable reference to a value,
...
then dereferencing `x{:rs}` gives you immutable access to the value.
