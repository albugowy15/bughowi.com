...
And let's just talk about it a little bit. So it's a little different than what you might have used before. So the first thing is that it has `href{:js}` prop, which is very similar to an href on anchor tag, except it actually doesn't take an href. So you're not gonna put a full URL here. Remember, this is client-side routing, so everything's gonna be pretty much relative here. It's relative to the pages directory.
...
If you need to add data fetching to a page component, you can do so by exporting a special function inside page component file. In there you can export a function a function called `getStaticProps(){:js}`. It's job is to prepare props for this page. `getStaticProps(){:js}` is allowed to be asynchronous. In `getStaticProps(){:js}` you can execute any code that would executed during the build process. You can do whatever you want, for example fetch data from an api, or from a database. Whatever you did to get data, you need to return an object. In this object you can configure various things, but most importantly you typically set a `props{:js}` property here that holds another object which will be the props object you receive in your component function.
...
If you only have one layout for your entire application, you can customize `_app.js{:js}` and wrap your application with the layout.
