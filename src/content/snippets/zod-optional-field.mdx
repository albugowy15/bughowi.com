...
date: 2023-12-11
description: Make a string field optional when it's an empty string
categories:
  - typescript
  - zod
  - data validation
...
The `emptyStringToUndefined` variable uses `z.literal("")` schema to represent a literal empty string. The `transform` method is used to convert the empty string to `undefined`.
