...
Make sure you've downloaded and installed the [PlanetScale CLI](https://github.com/planetscale/cli#installation). To authenticate with the PlanetScale CLI, enter the following:
...
When working with Prisma and PlanetScale, it's advisable to follow the workflow of using `prisma db push` instead of `prisma migrate`. You can find additional information about `prisma db push` [here](https://www.prisma.io/docs/concepts/components/prisma-migrate/db-push).
