...
The project uses <Link href="https://github.com/nodejs/undici" external={true}>undici</Link> instead of the native `fetch` because it provides better control over HTTP behavior and reliable streaming support.
...
Starting from <Link href="https://nextjs.org/blog/next-16" external={true}>Next.js 16</Link>, Proxy replaces the old Middleware as the recommended way to intercept requests at the network boundary. The key difference is that Proxy runs on the **Node.js runtime** instead of the Edge Runtime, giving you access to the full Node.js API. The old `middleware.ts` is deprecated and will be removed in a future version.
...
If you want to see the full working implementation with an interactive demo UI, check out the <Link href="https://github.com/albugowy15/next-proxy" external={true}>next-proxy</Link> repository on GitHub.
